## tripwire/audit_ffi.nim — Defense 2 Part 3 FFI audit stub.
##
...
## stub exists so that:
##
##   1. `-d:tripwireAuditFFI` is a valid, documented define for the v0
##      release.
##   2. The facade (`src/tripwire.nim`) can wire the import
...
##   Part 2: opt-in activation gate for `import tripwire` (shipped in G2).
...
when defined(tripwireAuditFFI):
  {.hint: "FFI audit not implemented in v0; see Appendix B of the " &
    "design doc. Stub compiled under -d:tripwireAuditFFI; no pragma " &
    "scanning performed.".}
