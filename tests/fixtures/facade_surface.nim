...
##
## This fixture imports `tripwire` (the facade) and references a symbol
## from every module the facade is meant to re-export. `nim check`
## with `-d:tripwireActive` must succeed; if the facade drops a module
## from its export list, this file fails at compile time.
##
## Run via `nim check -d:tripwireActive tests/fixtures/facade_surface.nim`
...
# ---- errors ------------------------------------------------------------
when not declared(TripwireDefect):
  {.error: "facade missing TripwireDefect".}
...
# ---- intercept ---------------------------------------------------------
when not declared(tripwireInterceptBody):
  {.error: "facade missing tripwireInterceptBody".}
...
# `test` is the tripwire-flavored test template; `check` and `suite`
# come from the re-exported std/unittest backend.
when not declared(test):          {.error: "facade missing tripwire test".}
