...
##
## Compiles src/nimfoot/audit_ffi.nim under `-d:nimfootAuditFFI` in a
...
##   3. The facade wiring (`when defined(nimfootAuditFFI): import ...`)
...
suite "audit_ffi stub (H6.5, Defense 2 Part 3)":
  test "audit_ffi.nim compiles under -d:nimfootAuditFFI and emits the hint":
    # Must allow the file to compile standalone, which means giving it
    # the `nimfootAllowInactive` escape hatch via --path:src so
    # `nimfoot/...` resolves, but NOT firing Defense 1 on the
    # facade: this fixture imports audit_ffi directly, not via
    # nimfoot.nim, so activation guards don't fire.
    let target = RepoRoot / "src" / "nimfoot" / "audit_ffi.nim"
    let cmd = "nim check --hints:on --path:" & (RepoRoot / "src") &
      " -d:nimfootAuditFFI " & target & " 2>&1"
    var output: string
    var code: int
    {.noRewrite.}:
  ...
  test "audit_ffi.nim is a no-op WITHOUT the define":
    # Without -d:nimfootAuditFFI the module body is a `when`-guarded
