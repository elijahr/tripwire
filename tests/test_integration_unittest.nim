...
## only reports them via the process exit code. So when we need to assert
## that the `test:` lifecycle actually RAISES a nimfoot defect (rather
## than checking that the program exit code flipped to 1), we invoke the
## lifecycle steps directly from the outer `std_ut.test` body. The happy
## path is still exercised through the real nested `test:` form below.
import std/[unittest as std_ut, exitprocs, tables, options]
import nimfoot/[types, errors, timeline, sandbox, verify, intercept,
                integration_unittest]

# ---- Local test plugin ----
# The `it` plugin is a minimal shim so we can register a mock and record
# an interaction without pulling in any "real" plugin.
type
...
std_ut.suite "integration_unittest":
  ...
  std_ut.test "backend re-export exposes std/unittest under default build":
    # Under the default (no -d:nimfootUnittest2) the backend alias resolves
    # to std/unittest. The constant is compile-time, but we assert at
    # runtime for parity with other suites.
    when defined(nimfootUnittest2):
      std_ut.check backendName == "unittest2"
    else:
